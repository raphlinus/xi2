...
fn backspace_offset(text: &Rope, start: usize) -> usize {
    #[derive(PartialEq)]
    enum State {
        Start,
        Lf,
...
pub fn offset_for_delete_backwards(region: &SelRegion, text: &Rope) -> usize {
    if !region.is_caret() {
        region.min()
    } else {
        // TODO: restore xi-editor tab logic (requires config and view)
